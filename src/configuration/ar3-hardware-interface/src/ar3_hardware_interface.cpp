...
AR3HardwareInterface::on_init(const hardware_interface::HardwareInfo& info)
{
  ...
  for (size_t i = 0; i < JOINT_COUNT; ++i) {
    Joint j;
    j.name_ = prefix_ + "_joint_" + to_string(i);
    j.bias_name_ = prefix_ + "_joint_" + to_string(i) + "_bias";
...
vector<hardware_interface::StateInterface> AR3HardwareInterface::export_state_interfaces()
{
  ...
  for (auto& joint : joints_) {
    if (joint.enabled_) {
      ...
      state_interfaces.emplace_back(joint.bias_name_, "position", &joint.position_state_raw_);
    }
  }
  if (gripper_enabled_) {
...
vector<hardware_interface::CommandInterface> AR3HardwareInterface::export_command_interfaces()
{
  ...
  for (auto& joint : joints_) {
    if (joint.enabled_) {
      ...
      command_interfaces.emplace_back(joint.bias_name_, "position", &joint.position_bias_);
    }
  }
  if (gripper_enabled_) {
                                                           ...
                                                           const rclcpp::Duration& period)
{
  ...
  for (size_t i = 0; i < JOINT_COUNT; ++i) {
    ...
    joints_[i].position_state_ = joints_[i].position_state_raw_ + joints_[i].position_bias_;
